### 配置conda环境
电脑原系统本身就有`python3`环境，为避免不同项目因不同环境产生冲突，使用`conda`为不同项目提供不同的运行环境。
...
```
##### 使用Jupyter Notebook
安装`Jupyter`之后，进入所需环境之后，在命令行输入`jupyter notebook`，浏览器会自动打开。选择创建相关文件后，当前本地目录会生成一个`.ipynb`后缀名的文件。
**这个时候千万不要关闭终端，关了之后浏览器上的服务就没了。**
